...
public partial class ElasticSearch9Provider : ISearchProvider, ISupportIndexSwap, ISupportIndexCreate, ISupportSuggestions
{
    ...
    private static readonly ConcurrentDictionary<string, SemaphoreSlim> _documentTypeLocks = new(StringComparer.OrdinalIgnoreCase);
    ...
    public ElasticSearch9Provider(
        IOptions<SearchOptions> searchOptions,
        IOptions<ElasticSearch9Options> elasticOptions,
    ...
    public virtual async Task SwapIndexAsync(string documentType)
    {
        ...
        try
        {
            await ExecuteWithDocumentTypeLockAsync(documentType, async () =>
            {
                ...
                if (backupIndexName != null)
                {
                    ...
                    bulkAliasDescriptorActions.Add(new AddAction { Index = backupIndexName, Alias = activeIndexAlias, IsWriteIndex = true });
                }

                bulkAliasDescriptorActions.Add(new AddAction { Index = activeIndexName, Alias = backupIndexAlias, IsWriteIndex = true });
    ...
    public virtual async Task AddActiveAlias(IEnumerable<string> documentTypes)
    {
        ...
        try
        {
            foreach (var documentType in documentTypes)
            {
                ...
                var aliasResponse = await Client.Indices.PutAliasAsync(indexName, indexAlias, x => x.IsWriteIndex(true));
    ...
    protected async Task<bool> CheckMLPipeline(string pipelineName)
    {
    ...
    protected async Task CreateMLPipeline(string pipelineName)
    {
        ...
        {
            throw new SearchException($"ML pipeline {pipelineName} can't be created. You can create the pipeline manually.", pipelineResult.ApiCallDetails.OriginalException);
        }
    }
    ...
    {
        var aliasResponse = await Client.Indices.GetAsync(new GetIndexRequest(indexAlias));
        if (!aliasResponse.IsValidResponse)
        {
    ...
    protected virtual FluentDictionaryOfNameAlias ConfigureIndexAliases(FluentDictionaryOfNameAlias dictionary, string documentType, string indexName, string alias)
    {
        ...
        {
            dictionary.Add(alias, new AliasDescriptor().IsWriteIndex(true));
        }
    ...
    protected async Task<string> GetIndexNameAsync(string indexAlias)
    {
    ...
    protected virtual async Task ExecuteWithDocumentTypeLockAsync(string documentType, Func<Task> action)
    {
        var semaphore = _documentTypeLocks.GetOrAdd(documentType, static _ => new SemaphoreSlim(1, 1));
    ...
    protected virtual async Task<TResult> ExecuteWithDocumentTypeLockAsync<TResult>(string documentType, Func<Task<TResult>> action)
    {
        var semaphore = _documentTypeLocks.GetOrAdd(documentType, static _ => new SemaphoreSlim(1, 1));
        await semaphore.WaitAsync();
    ...
    protected void CheckClientCreated()
    {
        if (Client == null)
        {
            throw new SearchException("Elastic Search 9 Provider connection is not configured. Please configure it in Search:ElasticSearch9 app configuration section.");
        }
    }
